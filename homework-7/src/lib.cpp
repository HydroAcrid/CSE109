...
//f32 height
//string name
vec pack109::serialize(Person item) {
  ...
  temp = pack109::serialize(item.age);
  for(u8 item : temp) {
    bytes.push_back(item);
  }
  temp.clear();
  ...
  temp = pack109::serialize(item.height);
  for(u8 item : temp) {
    bytes.push_back(item);
  }
  temp.clear();
...
Person pack109::deserialize_person(vec bytes) {
  ...
  while(i < bytes.size()) {
    ...
    if(key.compare("Age") == 0) {
      ...
      age = pack109::deserialize_u8(temp);
    }
      ...
      height = pack109::deserialize_f32(temp);
    }
